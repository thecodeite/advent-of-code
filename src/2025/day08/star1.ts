...
export function solve(input: Input) {
  ...
  console.timeLog("star1");

  distances.sort((a, b) => a.distSq - b.distSq);

  for (let wireCount = 0; wireCount < 1000; wireCount++) {
    ...
    //   wireCount,
    //   "circuits:",
    //   circuits.map(circuit => circuit.toString()).join("\n"),
    //   "count:",
    //   countCircuits(circuits, input.lines),
    // );
  }

  circuits.sort((a, b) => b.cords.length - a.cords.length);

  const largestCircuit = circuits[0];
  const nextLargestCircuit = circuits[1];
  const thirdLargestCircuit = circuits[2];

  return (
    largestCircuit.cords.length *
    nextLargestCircuit.cords.length *
    thirdLargestCircuit.cords.length
  );
}

function countCircuits(circuits: Circuit[], lines: Input["lines"]) {
  let count = 0;

  for (const line of lines) {
    const circuit = circuits.find(circuit => circuit.cords.includes(line.cord));
    if (!circuit) {
      count++;
    }
  }

  return count + circuits.length;
}
